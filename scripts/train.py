...
# from hisup.detector_GSA import BuildingDetector
from hisup.detector_fds_Copy1 import BuildingDetector
...
def train(cfg):
    ...
    for epoch in range(start_epoch+1, arguments['max_epoch']+1):
        ...
        for it, (images, annotations) in enumerate(train_dataset):
            ...
            optimizer.zero_grad()
            with autocast():                               # ← FP16 前向上下文
                loss_dict, _ = model(images, annotations) # ← 前向 & 计算损失
                total_loss = loss_reducer(loss_dict)      # ← 加权求和

            with torch.no_grad():                         # ← 日志部分保持 FP32
                loss_dict_reduced = {k: v.item() for k, v in loss_dict.items()}
                loss_reduced = total_loss.item()
                meters.update(loss=loss_reduced, **loss_dict_reduced)

            scaler.scale(total_loss).backward()           # ← 梯度缩放 + 反传
