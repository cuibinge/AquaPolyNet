...
class Foundation_Decoder_v1_loss6(BaseModule):
    ...
    def __init__(self,
        ...
        # 新增：填充损失-------------------------------------------------------------------------
        if rect_fill_cfg is not None:
            self.rect_fill_loss = MODELS.build(rect_fill_cfg)
            # 从配置中获取权重，如果没有则使用默认值
            self.rect_fill_weight = rect_fill_cfg.get('weight', 
                                    getattr(self.rect_fill_loss, 'weight', 0.4))
        else:
            self.rect_fill_loss = None
        # 新增：填充损失-------------------------------------------------------------------------
    ...
    def loss_by_feat(self, seg_logits: Tensor,
        ...
        if data_type == 'only_cd_label':
            ...
            # 填充损失 - 只对前景（建筑物B）应用
            if self.rect_fill_loss is not None:
                fill_loss = self.rect_fill_loss(b_logits)
                loss_dict['loss_rect_fill'] = fill_loss
            ...
            if self.rect_fill_loss is not None:
                fill_loss = self.rect_fill_loss(b_logits)
                loss_dict['loss_rect_fill'] = fill_loss * 0.5  # 权重减半
...
@MODELS.register_module()
class Foundation_Decoder_swin_v1_loss6(BaseModule):
