...
class CDVisualizationHook(SegVisualizationHook):
    ...
    def _after_iter(self,
        ...
        print(f"draw: {self.draw}, show: {self.show}")
        print(f"数据批次类型: {type(data_batch)}")
        print(f"输出数量: {len(outputs) if outputs else 0}")
        ...
        if self.every_n_inner_iters(batch_idx, self.interval):
            ...
            for output in outputs:
                ...
                if hasattr(output, "img_path") and output.img_path:
                    img_path = output.img_path[0] if isinstance(output.img_path, (list, tuple)) else output.img_path
                else:
                    img_path = getattr(output, "filename", f"result_{runner.iter}_{batch_idx}.png")
                    img_from_to = []
                    ...
                    if img_from_to and len(img_from_to) > 0:
                        ...
                        # 如果没有输入图像，创建一个有意义的背景
                        img = np.ones(self.img_shape) * 128  # 灰色背景
