...
class RandomMosaic_Modified(BaseTransform):
    ...
    def __init__(self,
        ...
        self.seg_pad_val = seg_pad_val
        self.ignore_type = ignore_type
        self.mosaic_debug_count = 0  # 添加调试计数器    
    ...
    #         if len(img_shape) == 3:
    #             mosaic_img = np.full(
    #                 (int(self.img_scale[0]), int(self.img_scale[1]), c),  # 使用实际通道数
    #                 self.pad_val,
    #                 dtype=results['img'][0].dtype)
    #         else:
    #             mosaic_img = np.full(
    ...
    #             img_i = result_patch['img'][img_idx]
    #             # 确保目标区域有正确的通道数
    #             target_region = mosaic_imgs[img_idx][y1:y2, x1:x2]
                
    #             # 如果通道数不匹配，进行调整
    #             if len(target_region.shape) == 3 and len(img_i.shape) == 3:
    #                 if target_region.shape[2] != img_i.shape[2]:
    #                     # 通道数不匹配，调整源图像
    #                     if target_region.shape[2] > img_i.shape[2]:
    #                         # 源图像通道少，填充
    #                         padded_img = np.zeros((img_i.shape[0], img_i.shape[1], target_region.shape[2]), 
    #                                             dtype=img_i.dtype)
    #                         padded_img[:, :, :img_i.shape[2]] = img_i
    #                         for ch in range(img_i.shape[2], target_region.shape[2]):
    #                             padded_img[:, :, ch] = img_i[:, :, -1]  # 用最后一个通道填充
    #                         img_i = padded_img
    #                     else:
    #                         # 源图像通道多，裁剪
    #                         img_i = img_i[:, :, :target_region.shape[2]]
                
    #             # 复制图像数据
    #             mosaic_imgs[img_idx][y1:y2, x1:x2] = img_i[y1:y2, x1:x2]
    ...
    def _mosaic_transform_img(self, results: dict) -> dict:
        ...
        # 🔍 修改这里：强制使用4通道，而不是参考图像的通道数
        # reference_channels = results['img'][0].shape[2] if len(results['img'][0].shape) == 3 else 1
        reference_channels = 4  # 强制使用4通道
        
        # 🔍 调试：检查输入
        self.mosaic_debug_count += 1
        if self.mosaic_debug_count <= 10:  # 只打印前10次
            for i, img in enumerate(results['img']):
                if len(img.shape) == 3:
                    actual_channels = img.shape[2]
        
        # 为每个时相创建mosaic图像（强制4通道）
        for i in range(len(results['img'])):
            if len(results['img'][0].shape) == 3:
                mosaic_img = np.full(
                    (int(self.img_scale[0]), int(self.img_scale[1]), reference_channels),  # 使用4通道
                    self.pad_val,
                    dtype=results['img'][0].dtype)
            else:
                mosaic_img = np.full(
        ...
        for i, loc in enumerate(loc_strs):
            ...
            for img_idx in range(len(results['img'])):
                img_i = result_patch['img'][img_idx]
                target_img = mosaic_imgs[img_idx]
                ...
                if len(img_i.shape) == 3:
                    if img_i.shape[2] != 4:
                        # 如果不是4通道，修复为4通道
                        if img_i.shape[2] == 3:
                            # 3通道 -> 4通道：复制第3通道作为第4通道
                            channel_3 = img_i[:, :, 2:3]
                            img_i = np.concatenate([img_i, channel_3], axis=2)

                        elif img_i.shape[2] < 3:
                            # 少于3通道，填充到4通道
                            h, w = img_i.shape[:2]
                            img_i_4ch = np.zeros((h, w, 4), dtype=img_i.dtype)
                            img_i_4ch[:, :, :img_i.shape[2]] = img_i
                            for ch in range(img_i.shape[2], 4):
                                if img_i.shape[2] > 0:
                                    img_i_4ch[:, :, ch] = img_i[:, :, -1]
                            img_i = img_i_4ch
                ...
                # 确保目标区域大小匹配
                source_region = img_i[y1:y2, x1:x2]
                target_region = target_img[y1:y2, x1:x2]
                
                # 调整大小以匹配
                if source_region.shape != target_region.shape:
                    min_h = min(source_region.shape[0], target_region.shape[0])
                    min_w = min(source_region.shape[1], target_region.shape[1])
                    source_region = source_region[:min_h, :min_w]
                    target_region = target_region[:min_h, :min_w]
                
                # 复制数据
                target_img[y1:y2, x1:x2] = source_region
        ...
        # # 🔍 调试：检查输出
        # if self.mosaic_debug_count <= 10:
        #     # print(f"🔍 MOSAIC DEBUG [输出检查 {self.mosaic_debug_count}]:")
        #     for i, img in enumerate(mosaic_imgs):
        #         # print(f"   输出图像{i}: 形状={img.shape}, dtype={img.dtype}")
        #         if len(img.shape) == 3:
        #             if img.shape[2] != 4:
        #                 print(f"   ❌ 错误: 输出图像{i}只有{img.shape[2]}个通道!")
        #             else:
        #                 print(f"   ✓ 成功: 输出图像{i}有4个通道")
