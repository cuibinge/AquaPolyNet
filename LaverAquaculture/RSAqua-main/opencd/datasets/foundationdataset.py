...
#     def full_init(self):
#         super().full_init()
#         if self._test_mode and self._metainfo.get('classes') is None:
#             raise RuntimeError('dataset metainfo `classes` should be specified when testing')
...
#                 continue
#             lines_specific_data_lists.append(line.strip())
#
#         print(f"🔍 主数据列表文件+++++: {self.data_list}")
#         print(f"🔍 找到++++++++++ {len(lines_specific_data_lists)} 个子数据列表")
#         print("1+")
#         # 读取每个data_list，该循环遍历每个数据集的train/val/test的list
#         for specific_data_list_path in lines_specific_data_lists:
#             # print(f"🔍 正在处理子数据列表: {specific_data_list_path}")
#             # print(f"🔍 正在处理子数据列表: {lines_specific_data_lists}")
#             print("2+")
#             try:
#                 # Windows 兼容路径处理
#                 print("3+")
#                 if not osp.exists(self.data_list):
#                     # 尝试使用 data_root 构建完整路径
#                     print("4+")
#                     if self.data_root and not osp.isabs(specific_data_list_path):
#                         print("5+")
#                         specific_data_list_path = osp.join(self.data_root, specific_data_list_path)
#
#                 if osp.exists(self.data_list):
#                     print("6+")
#                     print("self.data_list:", self.data_list)
#                     print("specific_data_list_path:", specific_data_list_path)
#                     with open(specific_data_list_path, 'r', encoding='utf-8') as f:
#                         print("7+")
#                         lines = [line.strip() for line in f if line.strip() and not line.strip().startswith('#')]
#                     print("lines",lines)
#                     print(f"  ✅ 成功读取 {len(lines)} 行数据")
#
#                     name = osp.basename(osp.dirname(specific_data_list_path)) + '_' + \
#                            osp.splitext(osp.basename(specific_data_list_path))[0]
#                     print("8+")
#                     for data_pair in lines:
#                         print("9+")
#                         # 解析制表符分隔的数据
#                         parts = data_pair.split('\t')
#                         if len(parts) >= 5:
#                             print("10+")
#                             img_a, img_b, label_cd, label_a, label_b = parts[:5]
#
#                             # 清理路径中的多余空格
#                             img_a = img_a.strip()
#                             img_b = img_b.strip()
#                             label_cd = label_cd.strip()
...
#                             data_info['label_b'] = label_b
#
#                             if data_info['img_b_path'] == '**' and data_info['label_cd'] == '**' and data_info[
#                                 'label_b'] == '**':
#                                 data_info['type'] = 'only_building_label'
#                             elif data_info['label_a'] == '**' and data_info['label_b'] == '**':
#                                 data_info['type'] = 'only_cd_label'
...
#                         else:
#                             print("11+")
#                             print(f"  ⚠️ 跳过格式错误的数据行: {data_pair}")
#                 else:
#                     print("12+")
#                     print(f"  ❌ 文件不存在: {specific_data_list_path}")
#
#             except Exception as e:
#                 print("13+")
#                 print(f"  ❌ 处理文件 {specific_data_list_path} 时出错: {e}")
#                 continue
#
#         print(f"✅ 总共加载 {len(data_list)} 个数据样本")
#         return data_list
#
#     @classmethod
