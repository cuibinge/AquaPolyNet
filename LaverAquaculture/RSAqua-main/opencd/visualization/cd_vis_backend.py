...
class CDLocalVisBackend(LocalVisBackend):
    ...
    def add_image(self,
        ...
        """
        print(f"=== CDLocalVisBackend.add_image 被调用 ===")
        print(f"名称: {name}")
        print(f"图像形状: {image.shape if image is not None else 'None'}")
        print(f"图像数据类型: {image.dtype if image is not None else 'None'}")
        print(f"保存目录: {self._img_save_dir}")
        print(f"image_from: {image_from.shape if image_from is not None else 'None'}")
        print(f"image_to: {image_to.shape if image_to is not None else 'None'}")        
