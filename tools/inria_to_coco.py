...
#     input_image_path = './lyg/cut_2048/img/'
#     input_gt_path = './lyg/cut_2048/gt/'
#     save_path = './lyg/cut_2048/train_512'
...
#         image_data = io.imread(os.path.join(input_image_path, label_name + '.png'))
...
# 设置路径
input_image_path = './data/lyg/cut_2048/val/img/'
input_gt_path = './data/lyg/cut_2048/val/gt/'
save_path = './data/lyg/cut_2048/val'

# 创建输出图像目录
output_im_val = os.path.join(save_path, 'images')
if not os.path.exists(output_im_val):
    os.makedirs(output_im_val)
...
for g_id, label in enumerate(tqdm(input_label)):
    label_name = label.split('.')[0]  # 假设文件名不含扩展名以外的点
    print(f"Processing file: {label_name}")  # 打印正在处理的文件名，以便调试
    image_data = imread(os.path.join(input_image_path, label_name + '.png'))
    gt_im_data = imread(os.path.join(input_gt_path, label_name + '.png'))
    im_h, im_w, _ = image_data.shape

    # 将标签转为多边形
    polygons = bmask_to_poly(gt_im_data, 1)
    for poly in polygons:
        area = round(poly.area, 2)
        if area > 0:
            # 计算边界框
            bbox = [poly.bounds[0], poly.bounds[1],
                    poly.bounds[2] - poly.bounds[0], poly.bounds[3] - poly.bounds[1]]
            ...
            if bbox[2] > 5 and bbox[3] > 5:
                ...
                coor_list = mapping(poly)['coordinates']
                for part_poly in coor_list:
                    seg.append(np.asarray(part_poly).ravel().tolist())
                ...
                output_data_val['annotations'].append(anno_info)
                val_ob_id += 1

    # 保存图像信息
    val_im_name = label_name + '.png'
    image_info = {'id': val_im_id, 'file_name': val_im_name, 'width': im_w, 'height': im_h}
    output_data_val['images'].append(image_info)

    # 保存图像
    imsave(os.path.join(output_im_val, val_im_name), image_data)
    val_im_id += 1
...
if not os.path.exists(save_path):
    os.makedirs(save_path)
with open(os.path.join(save_path, 'annotation.json'), 'w') as f_json:
    json.dump(output_data_val, f_json)

# if __name__ == '__main__':
#     input_image_path = './lyg/cut_512/train/img'
#     input_gt_path = './lyg/cut_512/train/gt'
#     save_path = './lyg/cut_512/train/'
