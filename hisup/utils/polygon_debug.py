...
def get_junctions(jloc, joff, topk=300, th=0):
    ...
    scores, index = torch.topk(jloc, k=topk)  # 修复：使用 topk 而不是 topK
...
def ext_c_to_poly_coco(ext_c, im_h, im_w):
    ...
    if len(contours) == 0:
        logger.debug("ext_c_to_poly_coco 未找到轮廓!")
        return np.array([])
...
def inn_c_to_poly_coco(inn_c, im_h, im_w):
    ...
    trans_prop_mask = mask.copy()
    f_y, f_x = np.where(mask == 1)
    trans_prop_mask[f_y[np.where(f_y == min(f_y))], f_x[np.where(f_y == min(f_x))]] = 0
    trans_prop_mask[f_y[np.where(f_x == min(f_x))], f_x[np.where(f_x == min(f_x))]] = 0
    contours, _ = cv2.findContours(trans_prop_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    if len(contours) == 0:
        logger.debug("inn_c_to_poly_coco 未找到轮廓!")
        return np.array([])
...
# 简化多边形
def simple_polygon_new(init_poly, thres=10):
    if len(init_poly) == 0:
        logger.debug("simple_polygon_new: init_poly 为空!")
        return np.array([])
    if len(init_poly) < 3:
        logger.debug(f"simple_polygon_new: init_poly 只有 {len(init_poly)} 个点，少于最小要求 3 个!")
        return np.array([])
    ...
    # 保留角度在 60 到 120 度之间的点
    flag1 = np.roll((lines_ang >= 60) & (lines_ang <= 120), 1, axis=0)
    simple_poly = poly[flag1]
    
    if len(simple_poly) < 3:
        logger.debug(f"simple_polygon_new: 简化后 simple_poly 只有 {len(simple_poly)} 个点，返回原始输入!")
        return np.concatenate((init_poly, init_poly[0].reshape(-1, 2)))
...
def get_poly_crowdai(prop, mask_pred, junctions):
    ...
    logger.debug(f"Contours 数量: {len(contours)}")
    logger.debug(f"Hierarchy 形状: {hierarchy.shape if hierarchy is not None else 'None'}")
    ...
    if hierarchy is None or len(contours) == 0:
        logger.debug("get_poly_crowdai: 未找到有效轮廓!")
        return np.array([]), score, []

    for contour, h in zip(contours, hierarchy[0]):
        c = []
        if h[3] == -1:  # 外轮廓
            c = ext_c_to_poly_coco(contour, mask_pred.shape[0], mask_pred.shape[1])
        elif h[3] != -1 and cv2.contourArea(contour) >= 50:  # 内轮廓
            c = inn_c_to_poly_coco(contour, mask_pred.shape[0], mask_pred.shape[1])

        if len(c) > 3:  # 确保多边形有至少 4 个顶点
            init_poly = c.copy()
            ...
            init_poly = simple_polygon_new(init_poly, thres=10)

            if len(init_poly) == 0:
                logger.debug("get_poly_crowdai: 生成的 init_poly 为空!")
                logger.debug(f"Contour 形状: {contour.shape}, Hierarchy: {h}")
                continue
...
def get_poly_inria(prop, mask_pred, junctions, pid):
    ...
    if hierarchy is None or len(contours) == 0:
        logger.debug("get_poly_inria: 未找到有效轮廓!")
        return np.array([]), score, []
        ...
        if len(contour) > 2:
            ...
            if len(junctions) > 0:
                ...
                if len(u) > 2:
                    ppoly = junctions[u[np.argsort(ind)]]
                    init_poly = ppoly
            init_poly = simple_polygon_new(init_poly, thres=20)
            if len(init_poly) == 0:
                logger.debug("get_poly_inria: simple_polygon_new 返回空结果，跳过!")
                continue
...
def generate_polygon(prop, mask, juncs, pid, test_inria):
    logger.debug(f"generate_polygon 输入: prop={prop}, juncs 数量={len(juncs)}, pid={pid}")
    if not test_inria:
        poly, score, juncs_pred_index = get_poly_crowdai(prop, mask, juncs)
        if len(poly) == 0:
            logger.debug("get_poly_crowdai 返回空 poly!")
            return np.array([]), np.array([]), np.array([]), score, []
        ...
        if len(poly) == 0:
            logger.debug("get_poly_inria 返回空 poly!")
            return np.array([]), np.array([]), np.array([]), score, []
