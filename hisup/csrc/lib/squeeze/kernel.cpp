...
// public:
//     Point() {}
//     Point(float x, float y, float ang) : x_(x), y_(y), ang_(ang), used_(false) {}
//     inline float x() const { return x_; }
//     inline float y() const { return y_; }
//     inline float ang() const { return ang_; }
//     inline bool used() const { return used_; }
//     inline float &x() { return x_; }
//     inline float &y() { return y_; }
//     inline float &ang() { return ang_; }
//     inline bool &used() { return used_; }
// private:
//     float x_, y_, ang_;
//     bool used_;
// };
...
//                 for (int k = 0; k < map(xx, yy).size(); ++k) {
//                     Point& pt = map(xx, yy, k);
//                     if (isaligned(reg_ang, pt.ang(), prec) && !pt.used()) {
//                         reg.push_back(pt);
//                         pt.used() = true;
//                         flag = true;
//                         sumdx += cos(pt.ang());
...
//     for (size_t i = 0; i < reg_int.size(); ++i) {
//         float weight = confidence[i];
//         x += reg_int[i].x() * weight;
//         y += reg_int[i].y() * weight;
//         sum += weight;
//     }
...
//     rect.x = x;
//     rect.y = y;
//     rect.theta = reg_angle;

//     rect.dx = cos(rect.theta);
//     rect.dy = sin(rect.theta);
...
//     for (const auto& pt : reg_int) {
//         float l = (pt.x() - x) * rect.dx + (pt.y() - y) * rect.dy;
//         float w = -(pt.x() - x) * rect.dy + (pt.y() - y) * rect.dx;
...
//     float length = l_max - l_min;
//     if (length < 1e-6)
//         return false;

//     float aspect_ratio = rect.width / length;
//     if (aspect_ratio < 0.1 || aspect_ratio > 10)
//         return false;

//     rect.x1 = x + l_min * rect.dx;
//     rect.y1 = y + l_min * rect.dy;
//     rect.x2 = x + l_max * rect.dx;
//     rect.y2 = y + l_max * rect.dy
...
// //     float x = 0.0, y = 0.0, sum = 0.0;
// //     for (size_t i = 0; i < reg_int.size(); ++i) {
// //         float weight = confidence[i];  // 使用置信度作为权重
// //         x += reg_int[i].x() * weight;  // 加权平均 x 坐标
// //         y += reg_int[i].y() * weight;  // 加权平均 y 坐标
// //         sum += weight;                 // 累计权重
// //     }
// //     if (sum < 1e-6)  // 如果权重总和过小，说明区域点过少
// //         return false;
// //     x /= sum;        // 计算加权平均 x 坐标
// //     y /= sum;        // 计算加权平均 y 坐标

// //     rect.x = x;      // 设置矩形中心点 x 坐标
// //     rect.y = y;      // 设置矩形中心点 y 坐标
// //     rect.theta = reg_angle;  // 设置矩形方向
...
// //     // 计算矩形的宽度和长度
// //     rect.width = w_max - w_min;  // 矩形宽度
// //     rect.l_min = l_min;          // 矩形长度的最小值
// //     rect.l_max = l_max;          // 矩形长度的最大值
// //     rect.w_min = w_min;          // 矩形宽度的最小值
// //     rect.w_max = w_max;          // 矩形宽度的最大值

// //     // 检查矩形的宽高比，避免生成过于扁平的矩形
// //     float length = l_max - l_min;  // 矩形的长度
// //     if (length < 1e-6)  // 如果长度过小，直接返回失败
// //         return false;

// //     float aspect_ratio = rect.width / length;  // 计算宽高比
// //     if (aspect_ratio < 0.1 || aspect_ratio > 10)  // 如果宽高比超出合理范围，返回失败
// //         return false;
